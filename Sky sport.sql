...
SELECT team, ((wins * 100.0) / matches_played) AS wins_percent FROM football_1;

#7 Write an SQL query to show which team has maximum goals_scored and their count
select team, goals_scored from football_1 where (select max(goals_scored));

#8 Write an SQL query to show percent of draws with respect to matches_played round of to 2 digits by each team
select team, round(draws * 100.0 / matches_played,2) from football_1;

#9 Write an SQL query to show which team has minimum goals_scored and their count
select team, goals_scored from football_1 where (select min(goals_scored));
...
select team, cards_yellow, cards_red from football_2 where cards_red = 1 order by cards_yellow;
...
select team, goals_per90, assists_per90, goals_assists_per90 from football_2 order by goals_assists_per90 desc;
...
select team, goals_per90, assists_per90, goals_assists_per90 from football_2 order by goals_assists_per90 asc;
...
select team, shots, shots_on_target, shots_on_target_pct from football_2 where shots_on_target_pct < 30 order by shots_on_target_pct;

#27 Write an SQL query to show team, shots_per90, shots_on_target_per90 for team Belgium
select team, shots_per90, shots_on_target_per90 from football_2 where team = "belgium";
...
select team, goals_per_shot, goals_per_shot_on_target, average_shot_distance from football_2 order by average_shot_distance desc;

#29 Write an SQL query to show team, errors, touches for which errors is 0 and touches is less than 1500 
select team, errors, touches from football_2 where errors = 0 and touches < 1500;

#30 Write an SQL query to show team, fouls which has maximum number of fouls
select team, fouls from football_2 where (select max(fouls));

#31 Write an SQL query to show team, offisdes which has offsides less than 10 or greater than 20
select team, offsides from football_2 where offsides < 10 or offsides > 20;

#32 Write an SQL query to show team, aerials_won, aerials_lost, aerials_won_pct in descending order by aerials_won_pct 
select team, aerials_won, aerials_lost, aerials_won_pct from football_2 order by aerials_won_pct desc;

#33 Write an SQL query to show number of teams each group has
SELECT "group1", COUNT(team) FROM football_1 GROUP BY "group1";

#34 Write a SQL query to show team names group 6 has
select team, group1 from football_1 where group1 = 6;

#35 Write an SQL query to show Australia belongs to which group 
select team, group1 from football_1 where team = "Australia";

#36	Write an SQL query to show group, average wins by each group 
select group1, avg(wins) from football_1 group by group1;
...
select group1, expected_goal_scored from football_1 where expected_goal_scored = (select max(expected_goal_scored)) order by expected_goal_scored;

#38	Write an SQL query to show group, minimum exp_goal_conceded by each group in descending order by exp_goal_conceded 
select group1, expected_goal_scored from football_1 where expected_goal_scored = (select min(expected_goal_scored)) order by expected_goal_scored desc;
...
select group1, expected_goal_scored from football_1 where expected_goal_scored = (select avg(expected_goal_scored)) order by exp_goal_difference_per_90;

#40	Write an SQL query to show which team has equal number of goals_scored and goals_against 
select team from football_1 where goals_scored = goals_against;

#41 Write an SQL query to show which team has maximum players_used 
select team, players_used from football_2 where players_used = (select MAX(players_used))

#42 Write an SQL query to show team, players_used, avg_age, games, minutes where minutes less than 500 and greater than 200 
select team, players_used, avg_age, games, minutes from football_2 where minutes between 200 and 500;

#43 Write an SQL query to show all data of group_stats in ascending order BY points
select * from football_1 order by points;

#44 Write an SQL query to show ALL UNIQUE team in ascending order by team
select distinct team from football_1 order by team;

#45 Write an SQL query to show average avg_age of each group and arrange it in descending order by avg_age
select f1. group1, avg(f2.avg_age) from 
football_1 f1 join football_2 f2 on f1.team = f2.team 
group by group1 order by avg_age desc;

#46 Write an SQL query to show sum of fouls for each group and arrange it in ascending order by fouls
select f1.group1, sum(f2.fouls) 
from football_1 f1 join football_2 f2 on f1.team = f2.team order by fouls;

#47	Write an SQL query to show total number of games for each group and arrange it in descending order by games
select f1.group1, f2.games from football_1 f1 join football_2 f2 on f1.team = f2.team;
...
select f1.group1, sum(f2.players_used) from football_1 f1 join football_2 f2 on f1.team = f2.team order by players_used;

#49 Write an SQL query to show total number of offsides for each group and arrange it in ascending order by offsides
select f1.group1, sum(f2.offsides) from football_1 f1 join football_2 f2 on f1.team = f2.team order by offsides;

#50	Write an SQL query to show average passes_pct for each group and arrange it in descending order by passes_pct
select f1.group1, avg(f2.passes_pct) from football_1 f1 join football_2 f2 on f1.team = f2.team order by passes_pct desc;

#51	Write an SQL query to show average goals_per90 for each group and arrange it in ascending order by goals_per90
select f1.group1, avg(f2.goals_per90) from football_1 f1 join football_2 f2 on f1.team = f2.team order by goals_per90;